import { SurveyData, Question, SurveyOption, CreateSurveyDto, SurveyListItem } from '../types/survey';

// Transform frontend survey data to backend API format
export const transformToBackendFormat = (frontendData: SurveyData): CreateSurveyDto => {
  ...
  const transformed: CreateSurveyDto = {
...
export const ensureOptionsArray = (field: any): SurveyOption[] => {
  ...
  if (Array.isArray(field)) {
    ...
    return field.map((option: any, index: number) => {
  ...
  if (typeof field === "string") {
    try {
      ...
      const options = field.split(',').map((opt: string, index: number) => ({
        id: `opt_${index}`,
        label: opt.trim(),
...
const ensureNumber = (field: any, defaultValue: number = 0): number => {
...
const ensureBoolean = (field: any, defaultValue: boolean = false): boolean => {
...
const ensureDate = (field: any): string | null => {
...
export const transformToFrontendFormat = (backendData: any): SurveyData => {
  return {
    id: backendData.id,
    ...
    pages:
      backendData.pages?.map((page: any, index: number) => ({
        id: page.id,
        name: page.name,
...
const mapBackendQuestionType = (backendType: string): string => {
  const typeMap: Record<string, string> = {
    text: "text-input",
    email: "email",
    textarea: "textarea",
...
export const transformResponseData = (formData: Record<string, any>, surveyId: string, metadata: any = {}): any => {
...
export const transformSurveyListData = (backendSurveys: any[]): SurveyListItem[] => {
...
export const validateBackendData = (data: SurveyData): SurveyData => {
  const cleaned = { ...data };
