...
import { SurveyViewerProps, SubmissionData } from "../types/components";
import { Question, SurveyData, SurveyOption } from "../types/survey";

const SurveyViewer: React.FC<SurveyViewerProps> = ({
  surveyData,
  ...
  const handleSubmit = async () => {
    if (validateCurrentPage()) {
      ...
      try {
        ...
        const questionMap = allQuestions.reduce((acc: Record<string, Question>, question: Question) => {
        ...
        const submissionData: SubmissionData = {
          formData,
          surveyId: surveyData?.id || undefined,
  ...
  const getSubmitButtonStyles = useCallback((variant?: string) => {
  ...
  const renderQuestion = useCallback((question: Question) => {
    ...
    switch (question?.type) {
      ...
      case "matrix-multiple":
        const rows = question?.rows || [{ id: "row1", label: "Row 1", value: "row1" }];
        const columns = question?.columns || [{ id: "col1", label: "Column 1", value: "col1" }];
      ...
      case "ranking":
        return (
          <div key={questionId} className={`mb-6 ${questionWidth}`}>
            ...
            <div className="space-y-2">
              {question?.options?.map((option: SurveyOption, index: number) => (
                <div
                  key={option?.id}
